...
class FileDownloader {
     ...
     * @throws IOException  on problems with downloading
     */
    boolean copyURLToFile(URL src, File dest) throws IOException {
        if (dest.exists()) {
            LOGGER.info("Overwriting file at "+dest);
        }
    ...
    private boolean copyURLToFileWithFTP(URL src, File dest) throws IOException {
        ...
        try {
            ...
            throw new IOException("ERROR: problem connecting when downloading file.", e);
        }
        try {
            ...
            throw new IOException("ERROR: could not use binary transfer.", e);
        }
        InputStream in = null;
        ...
        try {
            ...
            throw new IOException("ERROR: problem downloading file.", e);
        } catch (IOException e) {
            dest.delete();
            try {
                ftp.logout();
     ...
     * passive mode.
     */
    private boolean copyURLToFileThroughURL(URL src, File dest) throws IOException {
        setProxyProperties();
        ...
        try {
            ...
            throw new IOException("ERROR: Problem downloading file: " + e.getMessage());
        }
        return true;
    }
