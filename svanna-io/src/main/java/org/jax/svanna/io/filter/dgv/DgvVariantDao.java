...
public class DgvVariantDao implements PopulationVariantDao, Closeable {

    private static final Logger LOGGER = LoggerFactory.getLogger(DgvVariantDao.class);
    ...
    public DgvVariantDao(GenomicAssembly assembly, Path dgvTsv) throws IOException {
        this.assembly = assembly;
        this.tabixReader = new TabixReader(dgvTsv.toString());
        this.chromosomes = tabixReader.getChromosomes();
    }

    private Collection<PopulationVariant> parseLine(String line) {
...
//        Set<ExpMethodType> discoveryMethods = ExpMethodType.decodeFromString(arr[8], ",");
        ...
        if (arr[5].equals("gain+loss")) {
            return List.of(
                    BasePopulationVariant.of(contig, Strand.POSITIVE, coordinateSystem, start, end, variantAccession, VariantType.DUP, 100 * observedGains / sampleSize, PopulationVariantOrigin.DGV),
                    BasePopulationVariant.of(contig, Strand.POSITIVE, coordinateSystem, start, end, variantAccession, VariantType.DEL, 100 * observedLosses / sampleSize, PopulationVariantOrigin.DGV));
        } else {
    ...
    public List<PopulationVariant> getAllItems() {
        throw new UnsupportedOperationException("Loading all DGV elements into memory is not supported");
    }

    @Override
    public Set<PopulationVariantOrigin> availableOrigins() {
        return Set.of(PopulationVariantOrigin.DGV);
    }

    @Override
    public List<PopulationVariant> getOverlapping(GenomicRegion query, Set<PopulationVariantOrigin> origins) {
        if (!origins.contains(PopulationVariantOrigin.DGV) || !chromosomes.contains(query.contigName())) {
            return List.of();
        }

        if (LOGGER.isTraceEnabled()) {
            LOGGER.trace("Fetching DGV features overlapping the region {}:{}-{}({})", query.contigName(), query.start(), query.end(), query.strand());
        }
        String line;
        TabixReader.Iterator iterator = tabixReader.query(
                tabixReader.chr2tid(query.contigName()),
        ...
        try {
            List<PopulationVariant> features = new LinkedList<>();
            while ((line = iterator.next()) != null) {
                features.addAll(parseLine(line));
            }
            return features;
        } catch (IOException e) {
            LOGGER.warn("Error fetching features for {}:{}-{}", query.contigName(), query.start(), query.end());
            return List.of();
        }
    }
}
